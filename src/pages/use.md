# 我都用什麼

## 硬體

- 💻 筆電: MacBook Pro 14' 2021, M1 Pro, 32G - 依舊是戰神
- 🖥️ 螢幕: ASUS VP32UQ - 最便宜的 4k 32吋
- ⌨️ 鍵盤: Leopold FC900RBT 珊瑚海 - 顏色真的超美
- 🖱️ 滑鼠: Logi M590 - 還在尋找適合的替代品
- 🔊 喇叭: Creative GS3 - 沒有空間只能放 sound bar
- 📱 手機: Pixel 8 - 堪用，預設沒辦法 Miracast 很討厭
- 🪑 椅子: Backbone Bear - 已經買了第三把同款
- 📷 相機: Sony A7C2 - 輕便才會想帶出門
- 📷 鏡頭 - 重量先決
  - Sony 16-35mm F/4 - 廣角自拍、風景都很棒
  - Sony 50mm F/1.8 - 自動對焦大光圈

---

## 電腦軟體

### 瀏覽器

- [Brave Browser](https://brave.com/) - 隱私優先、擋廣告很好用
- [Firefox](https://www.mozilla.org/firefox/) - 支持非 Chromium 體系，最近嘗試 [Zen](https://zen-browser.app/) 中
- [Google Chrome](https://www.google.com/chrome/) - 遇到相容性問題時使用

---

### 開發工具

- [Antigravity](https://antigravity.google/) - AI IDE，免費額度休閒夠用
- [Ghostty](https://ghostty.org/) - 時下最火紅 Terminal
- [Hex Fiend](https://hexfiend.com/) - 快速的 Hex 編輯器
- [ImHex](https://imhex.werwolv.net/) - 進階的 Hex 分析工具
- [Klogg](https://klogg.filimonov.dev/) - 看大檔案用這個
- [Visual Studio Code](https://code.visualstudio.com/) - 強大的程式碼編輯器，正在搬家到 [VSCodium](https://vscodium.com/)

---

### 系統工具

- [BetterDisplay](https://github.com/waydabber/BetterDisplay) - 螢幕解析度與亮度控制
- [BetterTouchTool](https://folivora.ai/) - 可以根據不同 App 設定快速鍵
- [DaisyDisk](https://daisydiskapp.com/) - 磁碟空間分析工具
- [dupeGuru](https://dupeguru.voltaicideas.net/) - 重複檔案搜尋與清理
- [eqMac](https://eqmac.app/) - 讓 soundbar 的人聲可以更凸顯
- [Ice](https://icemenubar.app/) - 隱藏系統列
- [Karabiner-Elements](https://karabiner-elements.pqrs.org/) - 改鍵軟體，讓 CapsLock 再次偉大
- [LinearMouse](https://linearmouse.app/) - 打遊戲必備
- [Pearcleaner](https://github.com/alienator88/Pearcleaner) - 移除 App 的工具
- [Raycast](https://www.raycast.com/) - 快捷啟動器
- [Stats](https://github.com/exelban/stats) - 系統狀態監控
- [TextSnipper](https://textsniper.app/) - 截圖轉文字
- [地牛Wake Up](https://eew.earthquake.tw/) - 地震預警

---

### 壓縮軟體

- [BetterZip](https://macitbetter.com/) - 可以預覽壓縮檔
- [Keka](https://www.keka.io/) - 輕量級壓縮與解壓縮工具
- [ImageOptim](https://imageoptim.com/) - 圖片壓縮工具，很久沒更新考慮轉到 [squoosh](https://www.npmjs.com/package/@squoosh/cli)

---

### 資安工具

- [KeePassXC](https://keepassxc.org/) - 密碼管理器，可以透過 syncthing 同步
- [LuLu](https://objective-see.org/products/lulu.html) - macOS 防火牆
- [VeraCrypt](https://www.veracrypt.fr/) - 加密虛擬硬碟

---

### 私人內網

- [Tailscale](https://tailscale.com/) - 建立私人內網
- [Syncthing](https://syncthing.net/) - E2EE 檔案同步
- [RustDesk](https://rustdesk.com/) - 開源遠端桌面取代 TeamViewer

---

### 筆記軟體

- [Notion](https://www.notion.so/) - 收錄網路文章、看板使用
- [Obsidian](https://obsidian.md/) - Markdown 筆記軟體

---

### 多媒體

- [OBS Studio](https://obsproject.com/) - 螢幕錄影串流軟體
- [VLC](https://www.videolan.org/vlc/) - 萬用播放器

---

### 通訊軟體

- [Discord](https://discord.com/) - 遊戲與社群的通訊軟體
- [Line](https://www.line.me/tw/) - 拜託不要在用了
- [Microsoft Teams](https://www.microsoft.com/microsoft-teams/) - 工作需求，難用到生氣
- [Signal](https://signal.org/) - 研究員的通訊軟體
- [Telegram](https://telegram.org/) - 佛心通訊軟體，API 串 Bot 無敵

---

### CLI

- [ffmpeg](https://ffmpeg.org/) - 影音轉碼與處理工具
- [Lima](https://github.com/lima-vm/lima) - 在 macOS 上運行 Linux VM，我用他來跑 container
- [Neovim](https://neovim.io/) - 現代化的 Vim 編輯器
- [nvm](https://github.com/nvm-sh/nvm) - Node.js 版本管理工具
- [ripgrep](https://github.com/BurntSushi/ripgrep) - 超高速文字搜尋工具（`rg`）
- [scrcpy](https://github.com/Genymobile/scrcpy) - Android 螢幕鏡像與控制工具
- [tmux](https://github.com/tmux/tmux) - terminal 視窗管理工具
- [uv](https://github.com/astral-sh/uv) - Python 版本管理工具
- [yt-dlp](https://github.com/yt-dlp/yt-dlp) - 下載影片神器
- [zsh](https://www.zsh.org/) - plugin 還是最齊全

---

## 手機 App

- [andOTP](https://github.com/andOTP/andOTP) - 開源 2FA 工具，考慮轉換到 [Aegis](https://getaegis.app/)
- [App Manager](https://muntashir.dev/AppManager/) - 應用程式管理工具
- [Aurora Store](https://auroraoss.com/aurora-store) - 免登入 Google Play Store
- [DevCheck](https://devcheck.app/) - 系統資訊檢視工具
- [KeePassDX](https://www.keepassdx.com/) - KeePass 系列 Android 密碼管理器
- [Lux Light Meter Pro](https://play.google.com/store/apps/details?id=com.doggoapps.luxlight) - 光度計量工具
- [MJ PDF](https://gitlab.com/mudlej_android/mj_pdf_reader) - 開源 PDF 閱讀器 
- [Neo Store](https://neostore.app/) - 開源 App 集中地
- [Obtainium](https://github.com/ImranR98/Obtainium) - 訂閱 App 更新
- [PingTools](https://pingtools.org/) - 網路診斷工具
- [PipePipe](https://pipepipe.dev/) - YouTube, NicoNico, BiliBili 無廣告播放器
- [Shizuku](https://shizuku.rikka.app/) - 提供系統級 API 的服務
- [Tasker](https://tasker.joaoapps.com/) - 自動化任務工具
  - [AutoInput](https://play.google.com/store/apps/details?id=com.joaomgcd.autoinput) - Tasker 自動操作 UI
  - [AutoNotification](https://play.google.com/store/apps/details?id=com.joaomgcd.autonotification) - Tasker 通知自動化
- [Termux](https://termux.com/) - Android terminal

---

## 雲端服務

### 自架

- [FreshRSS](https://freshrss.org/) - RSS 閱讀器
- [Immich](https://immich.app/) - 開源版 Google photo
- [Nextcloud](https://nextcloud.com/) - 私人雲端硬碟
- [Obsidian LiveSync](https://github.com/vrtmrz/obsidian-livesync) - 同步 Obsidian 筆記
- [Plausible](https://plausible.io/) - 取代 Google Analytics

---

### 訂閱

- [Adobe 攝影方案](https://www.adobe.com/creativecloud/photography.html) - 暫時離不開 Lightroom Classic
- [Spotify](https://www.spotify.com/) - 音樂串流服務

---

## 🪀 溜溜球

- OFFSET YoYo
  - OUTLIER 5 - 外觀美、空轉久，適合表演
  - XENON - 手感極佳，適合練習
  - Limitless - 做出意想不到的招式 但真的很吵
- MAGICYOYO
  - Y02 AURORA - 發光球是小時候的夢幻逸品